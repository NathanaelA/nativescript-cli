...
export class DestCopy implements IBroccoliPlugin {
  ...
  public rebuild(treeDiff: IDiffResult): void {	  
	...
	treeDiff.changedDirectories.forEach(changedDirectory => {
		...
		let packageJsonFilePath = path.join(changedDirectoryAbsolutePath, "package.json");
	...
	_.each(dependencies, dependency => shelljs.cp("-R", dependency.directory, path.join(this.outputRoot, path.basename(dependency.directory))));
